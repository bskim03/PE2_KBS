...
for i in range(3):
    ax2.annotate(coeff_to_formula(models[i + 3]), xy=[1540, -11.5 - 1.75 * i], fontsize=fontsize)
    ax2.annotate(f"R$^2$={r2_scores[i + 1]}", xy=[1540, -12.25 - 1.75 * i], fontsize=fontsize)
ax2.legend(loc="upper left", fontsize=fontsize, ncol=3)

# I-V measurement 표시
model_pos = Model(diode_model)
params = model_pos.make_params(isat=1e-5, vt=26e-3, n=1.0)
params['n'].min = 1
result_pos = model_pos.fit(cur[8:], voltage=vol[8:], params=params)
model_neg = Model(diode_model)
params = model_neg.make_params(isat=0, vt=26e-3, n=1.0)
params['n'].min = 1
